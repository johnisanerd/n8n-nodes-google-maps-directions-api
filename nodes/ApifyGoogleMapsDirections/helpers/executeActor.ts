...
import { ACTOR_ID } from '../ApifyGoogleMapsDirections.node';
import { buildActorInput } from '../ApifyGoogleMapsDirections.properties';
...
function shapeItem(
	...
	// simplified
	const directions = (item.directions ?? []) as Array<Record<string, any>>;
	const routes = directions.map((d) => ({
		via: d.via,
		distance: d.formatted_distance ?? d.distance,
		duration: d.formatted_duration ?? d.duration,
	...
	return {
		...
		bestDuration: item.best_duration,
		directionsCount: item.directions_count,
		mapsUrl: item.google_maps_directions_url,
		routes,
	};
}
